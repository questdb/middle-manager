...
const HELP_SECTIONS: &[(&str, &[(&str, &str)])] = &[
    ...
    (
        ...
        &[
            ...
            ("s", "Sort loaded window by current column (none → asc → desc)"),
            ("y / Y / c / j", "Copy cell / row TSV / column values / row as JSON"),
            ("f", "Freeze / unfreeze current column (pin when scrolling)"),
            ("h / H", "Hide current column / unhide all"),
            ("Space", "Toggle row selection"),
            ("a", "Clear row selection"),
            ("Ctrl+A / Ctrl+I", "Select all rows / invert selection (≤ 1M rows)"),
            ("w / W / = / ~", "Widen / narrow / reset all / autofit current column"),
            ("^ / $", "Jump to first / last visible column"),
            ("i", "Column info popup (type, sizes, encodings per row group)"),
            ("R", "Reload file from disk (preserves cursor, widths, selection)"),
            ...
            ("e / E / J", "Export row group CSV / full CSV / full NDJSON (respects selection)"),
