...
pub fn render(frame: &mut Frame, state: &CopyDialogState) -> Rect {
    let t = theme();
    let dbg = t.dialog_bg;
    let title = if state.is_move { " Rename/Move " } else { " Copy " };
    ...
    let dest_text = format!("{:<width$}", state.destination, width = layout.cw);
    dh::render_line(
        frame,
        layout.content,
        2,
    ...
    let ow_pad = layout.cw.saturating_sub(label_prefix.len() + ow_label.len());
    dh::render_line(
        frame,
        layout.content,
        5,
    ...
    // y=6..11: checkboxes
    dh::render_checkbox(frame, layout.content, 6, "Process multiple destinations", state.process_multiple, state.focused == CopyDialogField::ProcessMultiple, normal, highlight);
    dh::render_checkbox(frame, layout.content, 7, "Copy files access mode", state.copy_access_mode, state.focused == CopyDialogField::CopyAccessMode, normal, highlight);
    dh::render_checkbox(frame, layout.content, 8, "Copy extended attributes", state.copy_extended_attrs, state.focused == CopyDialogField::CopyExtendedAttrs, normal, highlight);
    dh::render_checkbox(frame, layout.content, 9, "Disable write cache", state.disable_write_cache, state.focused == CopyDialogField::DisableWriteCache, normal, highlight);
    dh::render_checkbox(frame, layout.content, 10, "Produce sparse files", state.produce_sparse, state.focused == CopyDialogField::ProduceSparse, normal, highlight);
    dh::render_checkbox(frame, layout.content, 11, "Use copy-on-write if possible", state.use_cow, state.focused == CopyDialogField::UseCow, normal, highlight);
    ...
    // y=13: separator
    dh::render_separator(frame, layout.area, layout.inner.y + 13, t.dialog_border_style());

    // y=14: Use filter checkbox
    dh::render_checkbox(frame, layout.content, 14, "Use filter", state.use_filter, state.focused == CopyDialogField::UseFilter, normal, highlight);

    // y=15: separator
    dh::render_separator(frame, layout.area, layout.inner.y + 15, t.dialog_border_style());
    ...
    dh::render_buttons(
        ...
        16,
        &[
            (&format!("{{ {} }}", btn_label), state.focused == CopyDialogField::ButtonCopy),
